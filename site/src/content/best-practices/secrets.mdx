...
**True story from this very repo.** An early `pr_flow/mcp.json` shipped with a literal Supabase access token (`sbp_…`) committed in plaintext. We caught it on day 0 of the site build. The token was rotated; the file now uses `${SUPABASE_ACCESS_TOKEN}`. If this can happen at the company publishing the playbook, it can happen anywhere. Set up scanning **before** you commit a real key.
</Callout>
